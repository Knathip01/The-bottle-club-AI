...
export default function RegisterForm() {
  ...
  return (
    <div className="relative w-full max-w-[640px] py-12">
        ...
        <div className="relative bg-white/40 backdrop-blur-[40px] p-8 sm:p-12 w-full shadow-[0_40px_100px_-20px_rgba(0,0,0,0.12)] border border-white/60 rounded-[3rem] transition-all duration-700 hover:shadow-[0_60px_120px_-30px_rgba(0,0,0,0.18)] overflow-hidden">
          ...
          <div className="relative flex flex-col items-center mb-10">
            ...
            <p className="text-stone-500 text-sm font-medium tracking-wide uppercase">
              {t('auth.have_account')} <Link href="/login" className="text-primary font-black hover:text-primary-hover transition-colors border-b-2 border-primary/20 hover:border-primary">{t('auth.login_title')}</Link>
            </p>
          </div>

          <form onSubmit={handleRegister} className="relative space-y-7">
            {error && (
              ...
              <div className="space-y-2.5">
                ...
                <div className="relative group/input">
                  <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
                    <User className="h-5 w-5 text-stone-300 group-focus-within/input:text-primary transition-colors" />
                  </div>
                  <input 
                    type="text" 
                    name="firstName"
            ...
            <div className="space-y-2.5">
              ...
              <div className="relative group/input">
                <div className="absolute inset-y-0 left-6 flex items-center pointer-events-none">
                  <Phone className="h-5 w-5 text-stone-300 group-focus-within/input:text-primary transition-colors" />
                </div>
                <input 
                  type="tel" 
                  name="phone"
            ...
            <p className="text-[10px] text-stone-400 mt-2 leading-relaxed px-5 uppercase font-bold tracking-wider">
            ...
            <button 
              type="submit"
              disabled={loading}
              className={`group relative w-full h-[72px] bg-stone-950 text-white font-black uppercase tracking-[0.3em] text-[10px] rounded-[2rem] overflow-hidden transition-all active:scale-[0.97] shadow-2xl hover:shadow-primary/30 mt-4 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              {/* Shimmer Effect */}
